...
| 没赶上领取截止 | 填了明确截止时间的蛋，一过点会自动转入「已错过」并记一条消息（误判可点卡片上的「✅ 其实领到了」改回）；没填 / 填「待定」的不会自动动，卡片常驻「😢 错过了」按钮，想清掉随时点 |
...
开启状态下，只要页面开着，**领取截止前 24 小时内**（已领取的蛋：使用截止前 24 小时内）会弹系统通知；点击通知会自动跳到该蛋所在的分类标签页（待领取 / 已领取 / 已过期）并高亮。同一颗蛋只提醒一次（刷新页面也不会重复弹）。已领取的蛋一旦过了使用截止，会自动转入「已过期」；填了明确领取截止的待领取蛋一过点也会自动转入「已错过」。这两种自动流转除了弹系统通知，**还会各记一条到赛博鸡蛋顶栏「💬 消息」板块**，不怕通知一闪而过漏掉。想改提醒提前量，编辑 `public/config.js` 里的 `remind.hours`。

### 字段说明

- **领取截止**：什么时候之前要领（决定紧急程度排序和横幅提醒；填了明确时间、一过点就自动转入「已错过」并弹提示）
- **使用截止**：领到之后什么时候之前要用（「已领取」状态下会继续跟踪；过了就自动转入「已过期」并弹提示）
- **状态**：待领取 → 已领取 → 已用完；没赶上标「已错过」，活动结束标「已过期」
- **自由填写**：截止时间支持 `9-8`、`9/8 18:00`、`明天 14点` 等写法，不填年份默认今年；不知道就填「待定」，卡片会显示「待确认 ❓」并进横幅提醒（「待定」或没填的待领取蛋不会自动流转，卡片上有「😢 错过了」按钮手动清掉）
...
所有数据存在 `data/eggs.json`（赛博鸡蛋）、`data/papers.json`（文献）、`data/sites.json`（网页）、`data/expenses.json`（花销）与 `data/messages.json`（消息中心），纯文本 JSON，可直接查看编辑：
