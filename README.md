...
1. 生成自签证书并启动 HTTPS（见 `start-with-https.ps1` 内注释；Git for Windows 自带 openssl）
2. 手机在电脑在线时访问 `https://<电脑IP>:8642`，证书告警选「继续」
3. 浏览器菜单 →「添加到主屏幕 / 安装应用」
4. 之后**即使电脑关机**，也可从主屏打开界面并把想法记进手机本地队列
5. 电脑再次开机且手机能访问到服务时，顶栏自动同步（打开 App / 联网 / 每 60 秒）
